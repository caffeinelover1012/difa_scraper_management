...
ORGANIZATION = "PP-NAP"
DATASET_NAME = "Purchase to Plate National Average Prices (PP-NAP)"
...
class PPNAP(BaseScraper):
    def __init__(self, dataset_id=None):
        ...
        self.setup_static_attrs({
            ...
            'about_info': ABOUT,
            # Dataset link and file format will be set after fetching the actual data
        })
    ...
    def extract_data_attributes(self, **kwargs):
        ...
        try:
            # Extract dataset links and last updated date
            dataset_links = []
            links = main_page_soup.find('section', id="primary").find('table').find_all('a')
            default_link = "https://www.ers.usda.gov/"
            for link in links:
                dataset_links.append(default_link + link.get('href'))
            self.res["dataset_link"] = dataset_links

            all_dates = []
            rows = main_page_soup.find('section', id="primary").find('table').find_all('tr')
            for row in rows[1:]:
                date_str = row.find_all('td')[1].get_text()
                all_dates.append(standardize(DATE_FMT, date_str))
            ...
            self.res["dataset_file_format"] = ["csv", "xlsx"]
